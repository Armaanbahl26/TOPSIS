...
import logging
import sys
def topsis(sys1,sys2,sys3,sys4,lengthsys):
    a=sys1
    if(a[-4:]!='.csv'):
    ...
    try:
        ...
        exit(0)
    if(lengthsys!=5):
        logging.warning('Incorrect number of parameters passed')
        exit(0)
    i,j=dataset.shape
    if(j<3):
...
topsis(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],len(sys.argv))
